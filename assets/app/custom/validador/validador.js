									...
									{"columnDefs": [
										{
											...
											render: function(data, type, full, meta) {
												var status = {
													...
													PROTECCION_LABORAL: {'title': 'SÍ, PROTECCIÓN LABORAL', 'class': 'kt-badge--warning text-white'}
												};
				...
				if (resultado === 'error') {
					...
					if (resultado.responseMessageOut.header.responseStatus.statusCode = "SUCCESS") {
						if (resultado.responseMessageOut.body.response.validadorResponse !== 'undefined') {						
							...
							/** Ajuste periodo de protección laboral
							 * Ingeniero Gustavo Zuluaga
							 * Julio de 2021
							 */
							//VCalculo fecha actual - formato AAAAMMDD
							let date 	= new Date()
							let day 	= date.getDate()
							let month 	= date.getMonth() + 1
							let year 	= date.getFullYear()

							if(month < 10){ //hasta septiembre agrega un cero al mes, para tener ej: 20210920
								var hoy = year+'0'+month+''+day
							  }else{
								var hoy = year+''+month+''+day //20211220
							  }	
							...
							if(Afiliado.EstadoDescripcion == 'Retirado' && Afiliado.FechaRetiro >= hoy && hoy <= Afiliado.FechaRetiro)
							{
									...
									.draw()
									.node();
							}//Valida finalización periodo de protección laboral
							...
							{													
								...
								.draw()
								.node();
							}//Valida que se encuentre al Afiliado a Comfenalco
							else{
